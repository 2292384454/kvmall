...
func (service *VaildQQService) Vaild() serializer.Response {
	...
	if err := models.DB.Where("id=?", models.DB.Table("user_auth").Select("user_id").Where("identifier=?", result2.OpenID).SubQuery()).First(&user).Error; err != nil {
		//如果查询不到，则创建CMall账号
		if gorm.IsRecordNotFoundError(err) {
			...
			if err := models.DB.Create(&userAuth).Error; err != nil {
				logging.Info(err)
				code = e.ERROR_DATABASE
				return serializer.Response{
					Status: code,
